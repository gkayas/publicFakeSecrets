...
const { Text, TouchableWithoutFeedback, View, Platform } = ReactNative;
...
class SegmentedControls extends React.Component {
  ...
  renderOption(config, option, selected, onSelect, index) {
    ...
    const label = this.props.extractText
      ? this.props.extractText(option)
      : option;
    ...
    return (
      ...
      >
        <View
          style={[
            index > 0 ? separatorStyle : baseOptionContainerStyle,
            this.props.optionContainerStyle,
            borderRadiusStyle
          ]}
        >
          {typeof this.props.renderOption === "function" ? (
            this.props.renderOption.call(this, option, selected)
          ) : (
            <Text allowFontScaling={scaleFont} style={textStyle}>
              {label}
            </Text>
          )}
        </View>
